<template>
  <div id="gaodeMap">
    ...
    <div  class="topTips">
      <div v-for="(item, index) in mapList" :key="index"  class="topItem" @click="goBackCIty(item,index)" :class="['topItem',{'current':currentIndex === index }]">{{item}}</div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
      ...
      window.onLoad = function() {
        ...
        _this.map.on('click', function (ev) { // mousemove 鼠标移动 mouseover 鼠标覆盖 mouseout 鼠标离开
          ...
          if (obj.object) {
            ...
            if (_this.mapList.length < 3) { _this.mapList.push(object.adcodeYS) }
    ...
    goBackCIty(e, id) {
      ...
      console.log(this.currentIndex, 'dddddddddd')
